...
class HotelViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def nearby(self, request):
        ...
        try:
            ...
            features = []
            for hotel_obj, item in zip(hotels, serializer.data):
                ...
                props['icon'] = '🏨'
                props['distance_km'] = round(float(hotel_obj.distance) * 111, 2)
                features.append({
                    "type": "Feature",
                    "geometry": item['geometry'],
                    "properties": props,
    ...
    try:
        ...
        for poi in pois_data:
            ...
            if distance_km <= radius:
                ...
                for key in (
                    'stars', 'stars_label', 'price_range', 'price_hint',
                    'cuisine', 'opening_hours', 'takeaway', 'delivery',
                    'outdoor_seating', 'brand', 'operator', 'rooms', 'beds',
                    'email', 'internet_access', 'wheelchair', 'smoking',
                    'reservation', 'facilities', 'wikipedia', 'wikidata',
                ):
                    if key in poi:
                        props[key] = poi[key]
...
def approximate_location(request):
    """Approximate client location from public IP (fallback when GPS unavailable over HTTP)."""
    ...
    lookup_url = 'http://ip-api.com/json/?fields=status,lat,lon,city,country,message'
    if not is_private:
        lookup_url = f'http://ip-api.com/json/{ip}?fields=status,lat,lon,city,country,message'

    try:
        response = requests.get(lookup_url, timeout=5)
        data = response.json()
    except requests.RequestException as e:
        return Response({"error": f"Location lookup failed: {e}"}, status=503)

    if data.get('status') != 'success':
        return Response({"error": data.get('message', 'Location lookup failed')}, status=503)
