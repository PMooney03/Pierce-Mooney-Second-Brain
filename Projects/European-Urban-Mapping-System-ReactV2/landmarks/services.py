...
def query_overpass(query):
    """Execute an Overpass QL query, trying backup servers if needed."""
    headers = {
        "User-Agent": "EuropeanUrbanMappingSystem/1.0 (local development)",
    ...
    for url in OVERPASS_API_URLS:
        try:
            ...
            if 'remark' in result:
                print(f"Overpass API remark ({url}): {result['remark']}")
            if 'elements' not in result:
                print(f"Overpass API response missing 'elements' from {url}: {result}")
                continue
            ...
            if hasattr(e, 'response') and e.response is not None:
                try:
                    print(f"Overpass API error response: {e.response.text}")
                except Exception:
                    pass
        except Exception as e:
            print(f"Unexpected error querying Overpass API ({url}): {e}")
...
def format_osm_address(tags):
    ...
    line2 = ', '.join(
        filter(None, [tags.get('addr:city'), tags.get('addr:postcode'), tags.get('addr:country')])
    )
    if line1 and line2:
        return f'{line1}, {line2}'
    return line1 or line2 or tags.get('addr:street', '')

def extract_osm_extras(tags):
    """Pull ratings, prices, contact info, and amenities from OSM tags when present."""
    extras = {}

    stars_raw = (
        tags.get('stars')
        or tags.get('rating:stars')
        or tags.get('tourism:stars')
        or tags.get('star_rating')
    )
    if stars_raw:
        try:
            extras['stars'] = int(float(stars_raw))
        except (ValueError, TypeError):
            extras['stars_label'] = str(stars_raw)

    price_range = tags.get('price_range')
    if price_range:
        extras['price_range'] = price_range

    charge = tags.get('charge') or tags.get('fee') or tags.get('payment')
    if charge:
        extras['price_hint'] = charge

    cuisine = tags.get('cuisine') or tags.get('cuisine:en')
    if cuisine:
        extras['cuisine'] = cuisine.replace(';', ', ')

    opening_hours = tags.get('opening_hours')
    if opening_hours:
        extras['opening_hours'] = opening_hours

    if tags.get('takeaway') == 'yes':
        extras['takeaway'] = True
    ...
    if tags.get('rooms'):
        extras['rooms'] = tags['rooms']
    if tags.get('beds'):
        extras['beds'] = tags['beds']
    ...
    if internet_access:
        extras['internet_access'] = internet_access

    wheelchair = tags.get('wheelchair')
    if wheelchair:
        extras['wheelchair'] = wheelchair

    smoking = tags.get('smoking')
    if smoking:
        extras['smoking'] = smoking

    if tags.get('reservation') == 'yes':
        extras['reservation'] = True

    facilities = []
    if tags.get('swimming_pool') == 'yes':
        facilities.append('Pool')
    if tags.get('spa') == 'yes':
        facilities.append('Spa')
    if tags.get('sauna') == 'yes':
        facilities.append('Sauna')
    if tags.get('air_conditioning') == 'yes':
        facilities.append('A/C')
    if tags.get('breakfast') == 'yes':
        facilities.append('Breakfast')
    if facilities:
        extras['facilities'] = ', '.join(facilities)

    if tags.get('wikipedia'):
        extras['wikipedia'] = tags['wikipedia']
    if tags.get('wikidata'):
        extras['wikidata'] = tags['wikidata']
